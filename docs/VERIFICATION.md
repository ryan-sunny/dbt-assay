...
| The evaluator (`Sql/Semantics.lean`) means what SQL means | **Tested**, the same way: 22 named constructs and random queries against DuckDB (1 differs, integer division, DuckDB's defined behaviour, and certificates using it say so; where NULLs sort is resolved by the dialect's rule, as sqlglot resolves it) | as above |
| The statement: the theorem assay writes for a model is about THAT model (the join, the target, the columns) | **Tested on every run.** Each proven claim is run against the model itself: inputs generated to meet the certificate's premises and adversarial everywhere else, three datasets, the model's own SQL in DuckDB, the claim checked on its output. A claim that fails reads `contradicted`, never proven. On the 358-model warehouse: 351 of 433 held, 0 contradicted; the rest could not run (a UDF defined on the connection, generated values a function refuses) and say so. A project in another dialect runs sqlglot's DuckDB translation, kept only when it round-trips; its results say so, and a contradiction found only on a translation reads `unchecked`, because the translation is not proven to mean the same | a model whose SQL cannot run on generated inputs; three datasets missing the case; a translation that round-trips and still means something else |
| The parse: the tree assay reasons about is the SQL's | **Proven** for 282 of 328 models on the field warehouse (Lean's own parser reads the same tree from the text, checked by the kernel; the fragment now takes subqueries, date units, `DISTINCT ON`, `UNION BY NAME`, `IGNORE NULLS`, lambdas and more), **tested** for most of the rest (the SQL and its printed parse agree on generated rows). `--parse-on warehouse` first has the real engine plan each model (`EXPLAIN`, which runs nothing) | a model outside the fragment; two parsers agreeing wrongly |
...
| `staging_does_only_staging_work` | **not checked against real data by a person** | four answers read on the field warehouse (2026-09-25): a staging model keeping only some permit types, and one keeping sales over $100,000, both `a_filter_that_changes_meaning` (0.93, 0.85), which is right; a grid expansion `business_logic` at 0.45, rightly unsure. Four is not a verification |
| `literal_is_a_business_rule` | **not checked against real data by a person** | v1 called Colorado's latitude bounds (`between 36.9 and 41.1`) a business rule at 0.71; v2 names a real place's extent as a fact of the data and answers `a_fact_of_the_data` at 0.65 and 0.84, and `sale_price >= 100000` a rule at 1.00. Four answers, not a verification |
| `one_grain_stated` | **not checked against real data by a person** | new; 14 marts on the field warehouse have a description and a grain the SQL produces |
| `intermediate_has_one_purpose` | **not checked against real data by a person** | new; asked where an intermediate model has three joins or five CTEs (32 of the field warehouse's) |
| `mart_is_one_kind_of_thing` | **not checked against real data by a person** | new; asked where a mart unions several arms (27) |
| `fact_or_dimension_and_named_so` | **not checked against real data by a person** | new; asked of every mart (85) |
| `the_clock_is_meant` | **not checked against real data by a person** | new; asked where `output_depends_on_the_clock` fired outside a future-date bound (68) |
| `union_should_collapse_duplicates` | **not checked against real data by a person** | new; asked of every UNION without ALL (14) |
...
| `arrival_time_column` | built with the incremental checks (G-D) and tested on a fixture project written in Snowflake SQL. Asked only where no loader column names the arrival; the two incremental models on the field warehouse have no high-water-mark filter, so it has had **nothing to ask there, and no answer has been read** |
