...
mutual
  ...
  inductive Expr where
    ...
    /-- `* EXCLUDE (a, b)`: every column of the source but these. -/
    | starExcept (qual : List Str) (cols : List Str)
    /-- `INTERVAL '120' DAY`, `INTERVAL '3 months'`: the count as written, the unit lowercased. -/
    | interval (count : Str) (unit : Str)
    /-- `x -> body`, `(x, y) -> body`. -/
    | lambda (params : List Str) (body : Expr)
    /-- `xs[i]`, the index as written (1-based where the engine is). -/
    | index (a : Expr) (i : Expr)
    /-- `[a, b]`, a list literal. -/
    | list (xs : ExprList)
    /-- `f(x IGNORE NULLS)`, or `f(x) IGNORE NULLS`: the same call. -/
    | ignoreNulls (f : Expr)
  ...
  /-- Each key, descending or not, and whether NULLs come first: written, or the dialect's
  default when not (the parser resolves it, the way sqlglot does). -/
  inductive OrderList where
    | nil
    | cons (e : Expr) (desc : Bool) (nullsFirst : Bool) (rest : OrderList)
...
def OrderList.ofList : List (Expr × Bool × Bool) → OrderList
  | [] => .nil
  | (e, d, nf) :: xs => .cons e d nf (ofList xs)
